      ...
      <Card padding="none" className="border-slate-200 rounded-sm bg-white overflow-hidden">
        <CardContent className="p-3">
          <div className="grid grid-cols-4 gap-y-4 gap-x-2">
            ...
            <Link href="/petugas/stok-barang" className="flex flex-col items-center gap-1.5 group">
            ...
            <Link href="/petugas/nilai-sampah" className="flex flex-col items-center gap-1.5 group">
        ...
        <div className="space-y-2">
            ...
            <Card key={item.id} padding="none" className="overflow-hidden border-slate-100 hover:border-violet-100 transition-colors">
              <CardContent className="p-3 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-500 font-medium tracking-tight uppercase">{item.kode_penukaran} • {item.detail?.length || 0} Item</p>
                </div>
                <Link href={`/petugas/scan?code=${item.kode_penukaran}`}>
                  <Button size="sm" className="h-8 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-[10px] tracking-wider px-3 border-none">
                    Validasi
                  </Button>
                </Link>
              </CardContent>
          ...
          {!isLoadingQueue && queue.length === 0 && (
            <div className="text-center py-3 bg-white rounded-xs border border-slate-200">
               <p className="text-xs text-slate-400 italic tracking-widest">Tidak ada antrian hari ini</p>
            </div>
          )}
        </div>
      </div>
